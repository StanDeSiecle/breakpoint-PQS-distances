...
def find_nearest_g4(chromosome, position):
	...
	if len(possible_sites) < 1: return -1
	...
	#if closest > 65000: return -1
	return [closest_site, closest]

# Loads up the data from the two .csv files
def load_data():
	global g4s, recombs, in_trans_vars, in_trans_telomeric_vars, in_trans_non_vars, in_del_telomeric, in_dels;
	global t_in_trans_vars, t_in_trans_telomeric_vars, t_in_trans_non_vars, t_in_del_telomeric, t_in_dels;

	with open('data/3d7/3d7-g4s.csv', 'r') as g4csv:
	#with open('../../october-2015/g4_locations.csv','r') as g4csv:
		...
		for row in r:
			...
			# Exclude G4s that are not in subtelomeres?
			exclude = False
			if exclude:
				if int(sta) < 65000 or int(end) > chromosome_lengths[int(chm)-1] - 65000:
					g4s = g4s + [[chm,sta,end]]
			else:
				g4s = g4s + [[chm,sta,end]]
	...
	with open('data/3d7/3d7-breakpoints-all.csv', 'r') as recombscsv:
		...
		for row in r:
			...
			if nearest==-1:
				print "dropped chm " +str(chm) + " pos " + str(pos)
				continue

			# Type 1 is trans, type 2 is indel
			if typ==1:
			...
			if typ==2:
				...
				if tel=="Y":
					in_del_telomeric=in_del_telomeric+[[chm,pos,nearest[1],nearest[0]]]
					t_in_del_telomeric=t_in_del_telomeric+[nearest[1]]
	...
	in_del_telomeric = sorted(in_del_telomeric,key=lambda x:x[0])
...
#		print ", ".join(map(lambda x: str(x), r))
	...
	avg = find_average_distance( trials, null_type )
...
def run_experiments():
	...
	st_data_all = in_trans_telomeric_vars+in_del_telomeric
	t_st_data_all=t_in_trans_telomeric_vars+t_in_del_telomeric
	...
	e=e+do_experiment( "Subtelomeric all", st_data_all, t_st_data_all, num_trials, 1)
	e=e+do_experiment( "Subtelomeric all", st_data_all, t_st_data_all, num_trials, 2)
	# e=e+do_experiment( "Subtelomeric trans var", in_trans_telomeric_vars, t_in_trans_telomeric_vars, num_trials, 1)
	# e=e+do_experiment( "Subtelomeric trans var", in_trans_telomeric_vars, t_in_trans_telomeric_vars, num_trials, 2)
	# e=e+do_experiment( "Subtelomeric indel", in_del_telomeric, t_in_del_telomeric, num_trials, 1)
	# e=e+do_experiment( "Subtelomeric indel", in_del_telomeric, t_in_del_telomeric, num_trials, 2)
...
ax1.set_ylabel("Distance from nearest G4 (Kbp)")
ax1.set_yscale('linear')
ax1.set_ylim(0,100000)
ax1.yaxis.set_major_formatter(FuncFormatter(megabases))

ax1.boxplot(e[0],labels=labels,showmeans=True)
ax1.set_title("Subtelomeric breakpoints (n="+str(len(e[1][0]))+")")
...
plt.savefig("output/subtel-include-g4s-outside-1.png")
plt.figure(2)
...
ax1.set_title("Subtelomeric breakpoints (n="+str(len(e[0][0]))+")")

plt.suptitle("Measured breakpoint distance from G4 vs random sample distance from G4\n(probability of chromosome selection proportional to size)")
plt.savefig("output/subtel-include-g4s-outside-2.png")
#plt.show()
